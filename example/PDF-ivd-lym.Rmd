...
# load("RData/data-IVD.RData")
# load("RData/data-Lymph.RData")
...
legend("bottomright", 
       bty='n',
       legend = c(TeX(paste0("$p = 0.9, \\, \\hat{c}_1^2 =", sprintf("%.3f", est2[6,2]), #round(est2[6,2],3), 
                             ",\\, \\hat{c}_2^2 = ", sprintf("%.3f", est2[7, 2]), "$")), 
                  TeX(paste0("$p = 0.7, \\, \\hat{c}_1^2 =", sprintf("%.3f", est2[6,4]), 
                             ",\\, \\hat{c}_2^2 = ", sprintf("%.3f", est2[7,4]), "$")), 
                  TeX(paste0("$p = 0.5, \\, \\hat{c}_1^2 =", sprintf("%.3f", est2[6,6]), 
                             ",\\, \\hat{c}_2^2 = ", sprintf("%.3f", est2[7,6]), "$")), 
                  TeX(paste0("$p = 0.3, \\, \\hat{c}_1^2 =", sprintf("%.3f", est2[6,8]), 
                             ",\\, \\hat{c}_2^2 = ", sprintf("%.3f", est2[7,8]), "$"))
                  ), 
...
# SAUC <- sprintf("%.3f [%.3f, %.3f]", est[10, ],est[13, ], est[14,])
  ...
  SAUC <- sprintf("%.3f", est[10, ])
# c    <- tb[,6:7]
# sep  <- tb[,11:12]
  
est.m <- matrix(sprintf("%.3f", est[-c(10,13,14),]), ncol = 10)

res <- t(rbind.data.frame(SAUC, est.m))

tb <- cbind(p.seq, res)#[, -c(8:9, 13:14, 18:19)]

colnames(tb) <- c("$p$", "SAUC", "$\\mu_1$", "$\\mu_2$", "$\\tau_1$","$\\tau_2$","$\\tau_{12}$",
