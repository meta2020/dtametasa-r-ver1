---
title: "Appendix Table: estimates of other parameters when c11 = 0"
subtitle: "Scenario 1, 2, 3, 4, 9, 10, 15, 16"
author: "Yi"
...
table.pars <- function(list.n, row.n){
  ...
  true.par <- c(set[[list.n]][row.n, ][c(11, 2:6, 9:10, 7)])

  med  <- apply(DATA, 1:2, function(x) median(x, na.rm = TRUE))[c(11, 1:8),]
  iq25 <- apply(DATA, 1:2, function(x) quantile(x, prob =0.25, na.rm = TRUE))[c(11, 1:8),]
  iq75 <- apply(DATA, 1:2, function(x) quantile(x, prob =0.75, na.rm = TRUE))[c(11, 1:8),]
...
```{r}
s.rdt <- "scenario/18rows/set-0.5b-all-c01.RData"
dt <- "res/DT-pkg-0.5b-all-c01/"
```
...
colnames(longtab.f)[2:8] <- colnm


# sink("supp-c01-tab5-1.tex")
...
kbl(longtab.f, 
    ...
    escape = FALSE,
    caption = "Estimates of the parameters when $c_1^2 = 0, c_2^2 = 1$")
