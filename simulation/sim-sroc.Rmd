...
lines(plogis(ellipse::
               ellipse(-cor(sdata[,1:2])[3],
                       centre = colMeans(sdata[,1:2])[2:1]*c(-1,1),
                       # scale = c(1,1), 
                       scale = sqrt(var(sdata[,1:2])[c(4,1)]),
                       t = 1)), col="grey48", lty=1)

lines(plogis(ellipse::
               ellipse(-cor(rdata[,1:2])[3],
                       centre = colMeans(rdata[,1:2])[2:1]*c(-1,1),
                       scale = sqrt(var(sdata[,1:2])[c(4,1)]),
                       t = 1)),  col="grey48", lty=2)
...
legend('bottom',
       legend = c("published studies", "unpublished studies", 
                  "published region", "unpublished region",
                  paste0("p = ", c(1, 0.9, 0.7, 0.5)))[legend_order], 
       col = c("black", "black", "grey48", "grey48", 1:4)[legend_order], 
       lty = c(rep(0,2), 1, 2, rep(1,4))[legend_order],
       pch = c(1, 4, rep(0,2), rep(18,4))[legend_order],
       xpd = TRUE, 
       #horiz = TRUE, 
       cex = 1.2,
       pt.cex =c(1.2, 1.2,rep(0,2), rep(2,4))[legend_order],
       #seg.len=1, 
       bty = 'n',
       ncol = 4)
