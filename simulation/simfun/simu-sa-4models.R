...
##           SA1: THE PROPOSED MODEL (GIVEN C1=C2) 
...
simu <- function(list.n, 
  ...
  ml.p <- ml.s <- ml.sa1 <- ml.sa2<- rep(NA, 10)
  ...
  ## 3. ml.sa1 FOR SDATA (ADJUST PB) -------------------------------------------

  sa1 <- try(dtametasa.fc(data=sdata, 
                     p = p.e,   
                     brem.init = start0,
                     b.init = b0, 
                     c1.sq = csq, 
  ...
  if(!inherits(sa1, "try-error")) {

    if(sa1$convergence == 0) {
      
      ml.sa1 <- c(sa1$par[-c(10:12)], sa1$convergence)  
  ...
  x   <- cbind(ml.sa2, ml.sa1, ml.s, ml.p)
